...
const ACCESS_TOKEN_EXPIRY = process.env.ACCESS_TOKEN_EXPIRY || DEFAULT_ACCESS_TOKEN_EXPIRY; // e.g., 15 minutes
...
const REFRESH_TOKEN_EXPIRY = process.env.REFRESH_TOKEN_EXPIRY || DEFAULT_REFRESH_TOKEN_EXPIRY; // e.g., 7 days

// --- ADDED LOGGING TO VERIFY ENV VARS ---
    ...
    jwt.verify(refreshToken, REFRESH_TOKEN_SECRET, (err, decoded) => {
        ...
        db.get("SELECT user_id, username, refresh_token FROM users WHERE user_id = ?", [userId], (dbErr, user) => {
            ...
            if (user.refresh_token !== refreshToken) {
                 console.log(`Refresh failed: Provided token does not match stored token for user ${userId}.`);
        ...
        try {
            ...
            db.serialize(() => {
                ...
                db.run("UPDATE users SET password_hash = ? WHERE user_id = ?", [newPasswordHash, userId], function (updateErr) {
                    if (updateErr) { console.error(`DB error updating password for ${username}:`, updateErr.message); success = false; }
                    else if (this.changes === 0) { console.error(`Pwd change failed: No rows updated for user ID ${userId}.`); success = false; }
                });
                db.run("UPDATE users SET refresh_token = NULL WHERE user_id = ?", [userId], function (clearErr) {
                     if (clearErr) console.error(`DB error clearing refresh token after pwd change for ${username}:`, clearErr.message);
                     else console.log(`Refresh token cleared after password change for ${username}.`);
                });
                db.run(success ? "COMMIT;" : "ROLLBACK;", (commitErr) => {
                    if (commitErr) { console.error(`DB transaction error during pwd change for ${username}:`, commitErr.message); return res.status(500).json({ error: 'Failed to update password due to transaction error.' }); }
                    if (success) { console.log(`Password changed successfully for user: ${username}`); res.status(200).json({ message: 'Password changed successfully. Please log in again if needed.' }); }
                    else { res.status(500).json({ error: 'Failed to update password.' }); }
                });
            }); // End serialize
        } catch (error) {
...
app.listen(PORT, () => {
    ...
    // Security warnings already logged near top where vars are defined
